...
public class TDFFile implements ThirdPartyParseable
	...
	public void describeFormat(Parser prs) throws UnrecognizedFormatException
		{
		prs.stringEndingWith("\r\n", prs.property("numTunnels", int.class), false);
		for(int i=0; i<getNumTunnels();i++)
			{prs.subParseProposedClasses(prs.indexedProperty("tunnels", Tunnel.class, i), ClassInclusion.classOf(Tunnel.class));}
		}
	...
	public static class Tunnel implements ThirdPartyParseable
		...
		public void describeFormat(Parser prs) throws UnrecognizedFormatException
			...
			prs.subParseProposedClasses(prs.property("entrance", DirectionVector.class), ClassInclusion.classOf(DirectionVector.class));
			prs.subParseProposedClasses(prs.property("exit", DirectionVector.class), ClassInclusion.classOf(DirectionVector.class));
			prs.stringEndingWith("\r\n",prs.property("entranceLogic",TunnelLogic.class),false);
			prs.stringEndingWith("\r\n",prs.property("unused1", String.class),false);
			prs.stringEndingWith("\r\n", prs.property("entranceTerrainTextureFile", String.class), false);
		...
		/**
		 * @param tunnelLVLFile the tunnelLVLFile to set
		 */
