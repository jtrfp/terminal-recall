...
const uint OBJECT_TEXTURE_SIDE_LEN	= 512u;
const uint MATRICES_PER_ROW			= OBJECT_TEXTURE_SIDE_LEN/TEXELS_PER_MATRIX;
const uint VERTICES_PER_ROW			= MATRICES_PER_ROW+1u;

const float OBJ_TEX_HEIGHT_SCALAR	= 1/float(OBJECT_TEXTURE_SIDE_LEN);
const float OBJ_TEX_WIDTH_SCALAR	= float(TEXELS_PER_MATRIX)/float(OBJECT_TEXTURE_SIDE_LEN);
...
void main(){
 gl_Position.w          = 1;
 gl_Position.x			= dummy==1234?.000000001:0;
